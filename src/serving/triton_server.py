    ...
    def export_model(
        ...
        if isinstance(model, (VITS, MatchaTTS, BigVGANv2)):
            ...
            self._export_onnx_model(
                model, model_dir, model_name, input_shapes, batch_size
            )
            backend = "onnxruntime"

        # config.pbtxtの生成
        self._generate_config(
            model_name, backend, version, input_shapes, output_shapes, batch_size
        )
...
class TritonTTSClient:
    ...
    def synthesize_batch(
        ...
        if self.protocol == "grpc":
            ...
            inputs.append(
                grpcclient.InferInput("speaker_id", speaker_array.shape, "INT64")
            )
            ...
            speaker_array = np.array(speaker_ids, dtype=np.int64).reshape(-1, 1)
            inputs.append(
                httpclient.InferInput("speaker_id", speaker_array.shape, "INT64")
            )
            inputs[1].set_data_from_numpy(speaker_array)
