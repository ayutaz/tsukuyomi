...
try:
    ...
    warnings.warn("jaconv not available. Some Japanese text conversions will be limited.")
...
try:
    ...
    warnings.warn("text2phonemesequence not available. Phoneme conversion will be limited.")
...
class JapaneseTextNormalizer:
    ...
    def __init__(
        ...
        if TEXT2PHONEME_AVAILABLE:
            self.phonemizer = Text2PhonemeSequence(
                language='jpn',
                is_cuda=use_gpu
            )
        else:
        ...
        self.number_readings = {
            ...
            '10': 'ジュウ', '100': 'ヒャク', '1000': 'セン', '10000': 'マン'
        }
    ...
    def _normalize_numbers(self, text: str) -> str:
        ...
        text = re.sub(
            r'(\d{4})年',
            lambda m: self._number_to_japanese(m.group(1)) + 'ネン',
            text
        )
        ...
        text = re.sub(
            r'\d+',
            lambda m: self._number_to_japanese(m.group(0)),
            text
        )
