    ...
    def _filter_by_duration(self) -> List[AudioSample]:
        ...
        for sample in self.samples:
            try:
                ...
                logger.warning(f"Failed to read audio info for {sample.audio_path}: {e}")
                
        logger.info(f"Filtered from {len(self.samples)} to {len(filtered)} samples by duration")
        return filtered
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        for speaker_id in self.get_speaker_ids():
            ...
            stats["speakers"][speaker_id] = {
                ...
                "metadata": self.speaker_metadata.get(speaker_id).to_dict() if speaker_id in self.speaker_metadata else None
            }
