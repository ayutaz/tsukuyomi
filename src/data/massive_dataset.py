        ...
        try:
            ...
            if self.check_clipping:
                ...
                if clipping_ratio > 0.001:  # More than 0.1% samples clipped
                    return False, {"reason": "clipping", "clipping_ratio": clipping_ratio}
            ...
            if self.check_silence:
                ...
                if silence_ratio > 0.9:  # More than 90% silence
                    return False, {"reason": "too_silent", "silence_ratio": silence_ratio}
...
class MassiveGameVoiceDataset(Dataset):
    ...
    def _load_metadata(self):
        ...
        if cache_file.exists():
            ...
            # Filter by quality
            self.metadata = [
                item for item in full_metadata
        ...
        if self.load_audio:
            ...
            if self.segment_length is not None:
                ...
                if audio.shape[1] > segment_samples:
                    start = torch.randint(0, audio.shape[1] - segment_samples, (1,)).item()
    ...
    def get_speaker_stats(self) -> pd.DataFrame:
        ...
        for speaker_id in self.speaker_to_id:
            ...
            speaker_data.append({
                ...
                'avg_duration': np.mean([item['duration'] for item in speaker_items]),
                'avg_quality': np.mean([item['quality_score'] for item in speaker_items])
            })
...
class DistributedDataProcessor:
    ...
    def _calculate_stats(self, metadata: List[Dict]) -> DatasetStats:
        ...
        return DatasetStats(
            ...
            avg_quality=np.mean([m['quality_score'] for m in metadata]) if metadata else 0,
