    ...
    def forward(
        ...
        if use_memory:
            ...
            if new_memory_size > 0:
                self.memory_buffer[
                    :, self.memory_ptr : self.memory_ptr + new_memory_size
                ] = x[:1, -new_memory_size:]
                self.memory_ptr += new_memory_size
            else:
                # メモリが満杯の場合はFIFO
                self.memory_buffer = torch.cat(
                    [self.memory_buffer[:, x.shape[1] :], x[:1]], dim=1
                )
...
class StreamingVITS(nn.Module):
    ...
    def _replace_attention_layers(self):
        ...
        if hasattr(self.base_vits, "text_encoder") and hasattr(
            self.base_vits.text_encoder, "encoder"
        ):
    ...
    def decode_chunk(
        ...
        with torch.no_grad():
            ...
            if self.prev_overlap is not None:
                ...
                mel_chunk[:, :, :fade_len] = (
                    mel_chunk[:, :, :fade_len] * fade_in + self.prev_overlap * fade_out
                )
...
class StreamingPipeline:
    ...
    def _process_to_mel(self, text_chunk: str) -> torch.Tensor:
        ...
        return self.pipeline.process_text_chunk(
            text_chunk, self.current_speaker_id, self.current_style
        )
    ...
    def _initialize_caches(self):
        ...
        for name, module in self.base_vocoder.named_modules():
            if isinstance(module, nn.Conv1d):
                ...
                self.conv_caches[name] = torch.zeros(1, in_channels, kernel_size - 1)
