    ...
    def evaluate(
        ...
        if reference_audio is not None:
            ...
            if compute_all:
                # PESQ (requires 16kHz)
                try:
                    ref_16k = librosa.resample(reference_audio, orig_sr=sr, target_sr=16000)
                    syn_16k = librosa.resample(synthesized_audio, orig_sr=sr, target_sr=16000)
                ...
                # STOI
                try:
                    results['stoi_score'] = stoi(reference_audio, synthesized_audio, sr, extended=False)
                except Exception as e:
        ...
        if self.mos_predictor is not None:
            with torch.no_grad():
                audio_tensor = torch.from_numpy(synthesized_audio).float().unsqueeze(0).to(self.device)
    ...
    def save_results(
        ...
        data = {
            ...
            'summary': {
                ...
                'mean_speaker_sim': np.mean([r.speaker_similarity for r in results if r.speaker_similarity]),
                'mean_pronunciation': np.mean([r.pronunciation_accuracy for r in results if r.pronunciation_accuracy])
            }
        }
