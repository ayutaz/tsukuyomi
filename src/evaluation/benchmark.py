...
class PerformanceBenchmark:
    ...
    def _benchmark_configuration(
        ...
        logger.info(f"Testing: batch_size={batch_size}, seq_len={sequence_length}, device={device}")
    ...
    def _create_dummy_inputs(
        ...
        inputs = {
            'phoneme_ids': torch.randint(0, 100, (batch_size, sequence_length)).to(device),
    ...
    def _generate_summary_report(self, df: pd.DataFrame):
        ...
        report.append(f"- Mean RTF: {df['rtf_mean'].mean():.3f} ± {df['rtf_mean'].std():.3f}\n")
        ...
        report.append(f"- Throughput: {best_throughput['throughput']:.1f} samples/sec\n")
...
def benchmark_tts_system(
    ...
    if config.test_quantization:
        ...
        quantized_results = benchmark.benchmark_model(quantized_model, "pytorch_quantized")
