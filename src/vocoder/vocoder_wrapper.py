...
class HiFiGANVocoder(BaseVocoder):
    ...
    def _create_simple_hifigan(self) -> nn.Module:
        ...
        class SimpleHiFiGAN(nn.Module):
            def __init__(self, n_mels: int = 80, upsample_rates: list[int] = [8, 8, 2, 2]):
                super().__init__()
                ...
                self.resblocks = nn.ModuleList([
                    self._create_resblock(channels) 
                    for _ in range(3)
                ])
...
class UnifiedVocoder:
    ...
    def _auto_select_vocoder(self) -> VocoderType:
        ...
        if VOCOS_AVAILABLE:
            ...
            self.logger.warning("No vocoder libraries available, using built-in HiFi-GAN")
            return "hifigan"
    ...
    def from_checkpoint(cls, path: Path) -> "UnifiedVocoder":
        ...
        vocoder = cls(
            vocoder_type=checkpoint['vocoder_type'],
