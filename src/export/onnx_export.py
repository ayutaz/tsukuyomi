...
class ONNXExporter:
    ...
    def _validate_model(self, onnx_path: Path, sample_inputs: Optional[Dict[str, torch.Tensor]]):
        """
        ...
        # Run inference test
        try:
            providers = ['CUDAExecutionProvider'] if self.config.target_device == "gpu" else ['CPUExecutionProvider']
            ...
            if sample_inputs:
                ...
                logger.info(f"Validation successful. Output shapes: {[out.shape for out in outputs]}")
        except Exception as e:
    ...
    def _generate_metadata(self, onnx_path: Path, model_name: str, target: str):
        ...
        for input_meta in model.graph.input:
            metadata["input_info"][input_meta.name] = {
                "shape": [d.dim_value if d.dim_value > 0 else "dynamic" for d in input_meta.type.tensor_type.shape.dim],
                "dtype": onnx.TensorProto.DataType.Name(input_meta.type.tensor_type.elem_type)
            }
        
        for output_meta in model.graph.output:
            metadata["output_info"][output_meta.name] = {
                "shape": [d.dim_value if d.dim_value > 0 else "dynamic" for d in output_meta.type.tensor_type.shape.dim],
                "dtype": onnx.TensorProto.DataType.Name(output_meta.type.tensor_type.elem_type)
            }
