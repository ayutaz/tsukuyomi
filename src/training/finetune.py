...
def freeze_model_weights(
    model: nn.Module, config: FineTuningConfig, epoch: int = 0
) -> nn.Module:
...
def expand_speaker_embeddings(
    model: nn.Module, config: FineTuningConfig, base_n_speakers: int
) -> nn.Module:
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Embedding) and "speaker" in name:
            ...
            nn.init.normal_(
                new_embedding.weight.data[base_n_speakers:], mean=0.0, std=0.02
            )
...
def load_checkpoint_for_finetuning(
    ...
    if "n_speakers" in checkpoint:
        model = expand_speaker_embeddings(model, config, checkpoint["n_speakers"])
