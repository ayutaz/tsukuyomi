...
class MultiTaskLoss(nn.Module):
    ...
    def forward(
        ...
        # Duration loss
        if (
            self.use_duration_loss
            and "duration" in predictions
            and "duration" in targets
        ):
        ...
        if self.use_pitch_loss and "pitch" in predictions and "pitch" in targets:
            losses["pitch"] = self.pitch_loss(
                predictions["pitch"], targets["pitch"], targets.get("pitch_mask")
            )
        ...
        total_loss = torch.tensor(
            0.0, device=predictions.get("mel", torch.tensor(0)).device
        )
    ...
    def duration_loss(
        ...
        if mask is not None:
            ...
            return (
                F.mse_loss(pred_duration, target_duration, reduction="sum") / mask.sum()
            )
        else:
    ...
    def pitch_loss(
        ...
        if mask is not None:
            ...
            if voiced_mask.sum() > 0:
                ...
                return (
                    F.mse_loss(pred_pitch, target_pitch, reduction="sum")
                    / voiced_mask.sum()
                )
            else:
    ...
    def gan_losses(
        self, discriminator_outputs: Dict[str, List[torch.Tensor]]
        ...
        if (
            "fmap_real" in discriminator_outputs
            and "fmap_fake" in discriminator_outputs
        ):
            fm_losses = []
            for fmap_r, fmap_f in zip(
                discriminator_outputs["fmap_real"], discriminator_outputs["fmap_fake"]
            ):
    ...
    def forward(
        ...
        if emotion_embeddings is not None:
            losses["emotion_contrastive"] = self.contrastive_loss(
                emotion_embeddings, emotion_labels
            )
...
class StyleTransferLoss(nn.Module):
    ...
    def perceptual_loss(
        ...
        # Add log-scale loss
        loss += F.l1_loss(torch.log(pred_spec + 1e-7), torch.log(target_spec + 1e-7))
