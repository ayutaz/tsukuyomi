...
STYLES = ["normal", "energetic", "calm", "dramatic", "whisper", "shout", "formal", "casual"]
SPEAKERS = {
    "Default Female": 0,
    "Male Voice 1": 1,
    "Female Voice 2": 2,
...
def morph_voices(
    ...
    try:
        ...
        if emotions and emotion_weights:
            emotion_weights_list = [float(w.strip()) for w in emotion_weights.split(",")]
...
def create_waveform_plot(audio: np.ndarray, sr: int) -> plt.Figure:
    ...
    ax2.pcolormesh(t, f[:8000], 10 * np.log10(Sxx[:len(f[f<8000])]), 
        ...
        with gr.Tabs():
            ...
            with gr.TabItem("🎤 Basic Synthesis"):
                ...
                synthesize_btn.click(
                    fn=on_synthesize,
                    inputs=[
                        text_input, speaker_dropdown, emotion_dropdown,
                        style_dropdown, speed_slider, pitch_slider, energy_slider
                    ],
            ...
            with gr.TabItem("🎭 Voice Morphing"):
                ...
                with gr.Row():
                    emotions_select = gr.CheckboxGroup(
                        choices=EMOTIONS,
                        label="Select Emotions to Morph (optional)"
                    )
                ...
                morph_btn.click(
                    fn=on_morph,
                    inputs=[
                        morph_text, speakers_select, weights_input,
                        emotions_select, emotion_weights_input, morph_speed
                    ],
