...
if TRITON_AVAILABLE:
    ...
    def snake_beta_kernel(
        x_ptr, alpha_ptr, beta_ptr, output_ptr,
        n_elements, alpha_logscale: tl.constexpr,
...
class SnakeBeta(nn.Module):
    ...
    def __init__(self, in_features: int, alpha_logscale: bool = True, use_triton: bool = True):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_triton and x.is_cuda:
            ...
            snake_beta_kernel[grid](
                x_flat, alpha_broadcast, beta_broadcast, output,
                n_elements, self.alpha_logscale, BLOCK_SIZE
            )
        ...
        for dilation in dilations:
            ...
            self.convs1.append(
                weight_norm(nn.Conv1d(
                    channels, channels,
                    kernel_size, 1, padding, dilation=dilation
                ))
            )
            self.convs2.append(
                weight_norm(nn.Conv1d(
...
class BigVGANv2Generator(nn.Module):
    ...
    def inference(self, mel: torch.Tensor, use_cuda_graph: bool = True) -> torch.Tensor:
        ...
        Returns:
            Waveform (B, 1, T * hop_length)
        """
        if use_cuda_graph and torch.cuda.is_available() and hasattr(self, '_cuda_graph'):
    ...
    def compile_for_inference(self, example_mel: torch.Tensor) -> None:
        ...
        if cuda_version and float(cuda_version.split('.')[0]) >= 12.1:
            ...
            logger.warning(f"CUDA {cuda_version} detected - CUDA 12.1+ required for optimal compilation")
...
class BigVGANv2(nn.Module):
    ...
    def forward(
        ...
        if audio is not None and self.training:
            ...
            mpd_real, mpd_fake, mpd_feat_real, mpd_feat_fake = self.mpd(audio, audio_fake.detach())
            mrd_real, mrd_fake, mrd_feat_real, mrd_feat_fake = self.mrd(audio, audio_fake.detach())
