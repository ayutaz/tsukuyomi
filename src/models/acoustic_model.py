...
class AcousticModel(nn.Module):
    ...
    def __init__(self, config: AcousticModelConfig):
        ...
        self.input_projection = nn.Linear(
            config.phoneme_embedding_dim, config.hidden_dim
        )
        ...
        if config.n_speakers > 1:
            ...
            self.speaker_projection = nn.Linear(
                config.speaker_embedding_dim, config.hidden_dim
            )
        else:
        ...
        # Decoder projection
        self.decoder_projection = nn.Linear(config.hidden_dim, config.n_mel_channels)
    ...
    def forward(
        ...
        for block in self.encoder_blocks:
            if self.config.gradient_checkpointing and self.training:
                hidden = torch.utils.checkpoint.checkpoint(block, hidden, phoneme_mask)
            else:
        ...
        if target_durations is not None:
            ...
            predicted_durations = predicted_durations.clamp(
                min=1, max=100
    ...
    def _expand_states(
        self, hidden_states: torch.Tensor, durations: torch.Tensor
    ) -> torch.Tensor:
        """
        Expand hidden states according to durations.
        ...
        for b in range(batch_size):
            ...
            for t in range(seq_len):
                ...
                if duration > 0:
                    expanded.append(hidden_states[b, t : t + 1].expand(duration, -1))
    ...
    def __init__(self, hidden_dim: int, filter_size: int = 256, n_layers: int = 2):
        ...
        for i in range(n_layers):
            ...
            self.conv_layers.append(
                nn.Conv1d(in_channels, filter_size, kernel_size=3, padding=1)
            )
