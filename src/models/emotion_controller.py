...
class EmotionEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.emotion_embeddings = nn.Embedding(
            num_emotions, emotion_embedding_dim
        )
...
class EmotionFusion(nn.Module):
    ...
    def forward(
        ...
        if self.fusion_type == "concat":
            # 感情埋め込みを時間軸に拡張
            emotion_expanded = emotion_embedding.unsqueeze(1).expand(
                -1, seq_len, -1
            )
            ...
            attended, _ = self.cross_attention(
                emotion_query, features, features,
...
class EmotionController(nn.Module):
    ...
    def forward(
        ...
        # 特徴との融合
        fused_features = self.emotion_fusion(
            features, emotion_embedding, mask
        )
