        ...
        if interpolation_type == "learned":
            ...
            for i in range(num_layers):
                self.interpolation_net.add_module(
                    ...
                    nn.Sequential(
                        nn.Linear(
                            feature_dim * 2 + 1 if i == 0 else hidden_dim,
                            hidden_dim
                        ),
        ...
        if self.interpolation_type == "linear":
            ...
            if len(features) == 2:
                concat_features = torch.cat([
                    features[0], features[1], weights[:, 1:2]
                ], dim=-1)
                ...
                interpolated = features[0]
                for i in range(1, len(features)):
                    alpha = weights[:, i:i+1] / weights[:, :i+1].sum(dim=1, keepdim=True)
                    concat_features = torch.cat([
                        interpolated, features[i], alpha
                    ], dim=-1)
                    interpolated = self.interpolation_net(concat_features)
...
class VoiceMorphing(nn.Module):
    ...
    def morph(
        ...
        # デコード
        morphed_mel = self.decoder(content, interpolated_speaker, masks[0] if masks else None)
    ...
    def forward(
        ...
        for b in range(batch_size):
            for t in range(time_steps):
                shifted_mel[b, t] = self._interp1d(
                    freq_bins, mel[b, t], shifted_bins
                )
...
class ContinuousMorphing(nn.Module):
    ...
    def forward(
        ...
        for start in range(0, time_steps - self.window_size, self.hop_size):
            ...
            for i, (point, length) in enumerate(zip(transition_points, transition_lengths)):
            ...
            if in_transition and current_speaker_idx < len(target_speakers) - 1:
                # 2話者間の補間
                weights = torch.tensor([1 - transition_alpha, transition_alpha]).unsqueeze(0)
                speakers = [
                    target_speakers[current_speaker_idx],
                    target_speakers[current_speaker_idx + 1]
                ]
                morphed_window = self.morphing_module.morph(
                    [window], speakers, weights
                )
            else:
                # 単一話者
                morphed_window = self.morphing_module.morph(
                    [window], [target_speakers[min(current_speaker_idx, len(target_speakers)-1)]]
                )
            ...
            if start == 0:
                ...
                morphed_mel[:, start:start+overlap, :] = (
                    morphed_mel[:, start:start+overlap, :] * fade_out.unsqueeze(-1) +
                    morphed_window[:, :overlap, :] * fade_in.unsqueeze(-1)
                )
                morphed_mel[:, start+overlap:end, :] = morphed_window[:, overlap:, :]
