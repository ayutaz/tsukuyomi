...
if torch.cuda.is_available():
    ...
    if cuda_version and float(cuda_version.split(".")[0]) >= 12.1:
        logger.info(
            f"CUDA {cuda_version} detected - enabling XPhoneBERT-Japanese optimizations"
        )
...
class JapanesePhonemeAdapter(nn.Module):
    ...
    def __init__(self, config: XPhoneBERTJapaneseConfig):
        ...
        self.accent_embedding = nn.Embedding(
            config.num_accent_types, config.hidden_size
        )

        # Dialect embedding
        self.dialect_embedding = nn.Embedding(config.num_dialects, config.hidden_size)
    ...
    def forward(
        ...
        dialect_emb = (
            self.dialect_embedding(dialect_ids).unsqueeze(1).expand(-1, seq_len, -1)
        )
        ...
        combined = torch.cat(
            [hidden_states, accent_emb, dialect_emb + pitch_emb], dim=-1
        )
...
class XPhoneBERTJapanese(nn.Module):
    ...
    def __init__(self, config: Optional[XPhoneBERTJapaneseConfig] = None):
        ...
        try:
            self.base_model = AutoModel.from_pretrained(
                self.config.base_model, trust_remote_code=True, use_auth_token=False
            )
        ...
        self.japanese_phoneme_embeddings = nn.Embedding(
            100, self.config.hidden_size  # Extended vocabulary for Japanese phonemes
        )
    ...
    def forward(
        ...
        if phoneme_strings is not None and self.tokenizer is not None:
            ...
            encoded = self.tokenizer(
                phoneme_strings, padding=True, truncation=True, return_tensors="pt"
            )
        ...
        outputs = self.base_model(
            input_ids=phoneme_ids, attention_mask=attention_mask, return_dict=True
        )
    ...
    def encode_japanese_phonemes(
        ...
        if self.tokenizer:
            encoded = self.tokenizer(phonemes, return_tensors="pt")
            phoneme_ids = encoded["input_ids"]
        else:
    ...
    def save_pretrained(self, save_path: Path):
        ...
        if self.config.use_lora:
            lora_state_dict = {
                name: module.state_dict() for name, module in self.lora_layers.items()
            }
...
if __name__ == "__main__":
    ...
    for phrase in test_phrases:
        embeddings = model.encode_japanese_phonemes(
            phrase, accent_pattern="LHHHL", dialect="tokyo"
        )
