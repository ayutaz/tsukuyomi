...
def slice_segments(
    x: torch.Tensor, ids_str: torch.Tensor, segment_size: int
) -> torch.Tensor:
...
class MonotonicAlign:
    ...
    @staticmethod
    def maximum_path(
        neg_cent: torch.Tensor, x_mask: torch.Tensor, y_mask: torch.Tensor
    ) -> torch.Tensor:
...
def f0_to_coarse(
    f0: torch.Tensor, f0_bin: int = 256, f0_min: float = 50.0, f0_max: float = 1100.0
) -> torch.Tensor:
