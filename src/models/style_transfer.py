...
class StyleMixer(nn.Module):
    ...
    def forward(
        ...
        # スタック
        styles = torch.stack(
            style_embeddings, dim=1
        )  # (batch_size, num_styles, style_dim)
        ...
        if style_embedding is None:
            if reference_mel is not None:
                ...
                style_embedding = torch.zeros(
                    batch_size, self.style_encoder.style_dim, device=device
                )
...
class AdaptiveStyleTransfer(nn.Module):
    ...
    def forward(
        ...
        """
        # コンテンツ分析
        content_features = self.content_analyzer(
            features, src_key_padding_mask=content_mask
        )
