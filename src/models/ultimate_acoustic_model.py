...
if torch.cuda.is_available():
    ...
    if cuda_version and float(cuda_version.split('.')[0]) >= 12.1:
        ...
        logger.warning(f"CUDA {cuda_version} detected - CUDA 12.1+ required for optimal performance")
...
class FlowBlock(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(n_layers):
            ...
            self.ffns.append(
                FFN(channels, hidden_channels, kernel_size, p_dropout)
            )
...
class MultiSpeakerEncoder(nn.Module):
    ...
    def __init__(self, config: UltimateAcousticConfig):
        ...
        self.style_tokens = nn.Parameter(
            torch.randn(10, config.speaker_embed_dim)
        )
...
class MultiHeadAttention(nn.Module):
    ...
    def attention(
        ...
        query = query.view(b, self.n_heads, self.channels // self.n_heads, t_t).transpose(2, 3)
        key = key.view(b, self.n_heads, self.channels // self.n_heads, t_s).transpose(2, 3)
        value = value.view(b, self.n_heads, self.channels // self.n_heads, t_s).transpose(2, 3)
...
class FFN(nn.Module):
    ...
    def __init__(
        ...
        self.conv_2 = nn.Conv1d(hidden_channels, in_channels, kernel_size, padding=kernel_size//2)
        self.dropout = nn.Dropout(p_dropout)
...
class UltimateAcousticModel(nn.Module):
    ...
    def forward(
        ...
        if self.training and mel_lengths is not None:
            ...
            w, duration_loss = self.duration_predictor(
                x,
                phoneme_mask,
                w=mel_lengths.unsqueeze(1).float(),
                reverse=False
            )
        else:
    ...
    def morph_voices(
        ...
        # Validate weights
        if not torch.allclose(speaker_weights.sum(dim=1), torch.ones(batch_size, device=device), atol=1e-3):
        ...
        for i in range(n_speakers):
            ...
            speaker_embeddings.append(speaker_embed * speaker_weights[:, i].unsqueeze(-1))
        ...
        if emotion_ids is not None and emotion_weights is not None:
            if not torch.allclose(emotion_weights.sum(dim=1), torch.ones(batch_size, device=device), atol=1e-3):
            ...
            for i in range(n_emotions):
                ...
                emotion_embeddings.append(emotion_embed * emotion_weights[:, i].unsqueeze(-1))
    ...
    def compile_for_inference(self, example_input: Dict[str, torch.Tensor]) -> None:
        ...
        if cuda_version and float(cuda_version.split('.')[0]) >= 12.1:
            ...
            logger.warning(f"CUDA {cuda_version} detected - CUDA 12.1+ required for compilation")
...
def generate_path(durations: torch.Tensor, mask_x: torch.Tensor, mask_y: torch.Tensor) -> torch.Tensor:
...
def expand_durations(x: torch.Tensor, durations: torch.Tensor) -> torch.Tensor:
    ...
    for b_idx in range(b):
        ...
        for t_idx in range(t):
            ...
            if dur > 0:
                expanded[b_idx, :, curr_idx:curr_idx + dur] = x[b_idx, :, t_idx:t_idx + 1]
                curr_idx += dur
