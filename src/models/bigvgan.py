...
from .hifigan import Generator as HiFiGANGenerator, MultiPeriodDiscriminator, MultiScaleDiscriminator
...
class BigVGANResBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        for i, (c1, c2, act, aa) in enumerate(zip(
            self.convs1, self.convs2, self.activations, self.antialias
        )):
            xt = act(x)
            xt = c1(xt)
            xt = act(xt)
...
class BigVGAN(nn.Module):
    ...
    def forward(self, mel: torch.Tensor, return_discriminator: bool = False) -> torch.Tensor:
