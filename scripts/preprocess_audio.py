...
class AudioPreprocessor:
    ...
    def process_audio_file(
        self, input_path: Path, output_path: Path, transcript: Optional[str] = None
    ) -> Dict:
    ...
    def _reduce_noise(self, audio: np.ndarray, sr: int) -> np.ndarray:
        ...
        reduced_audio = nr.reduce_noise(
            y=audio, sr=sr, y_noise=noise_sample, prop_decrease=0.8
        )
    ...
    def _normalize_loudness(self, audio: np.ndarray, sr: int) -> np.ndarray:
        ...
        # Normalize
        audio_normalized = pyln.normalize.loudness(audio, loudness, self.target_db)

        # Prevent clipping
        if np.abs(audio_normalized).max() > 0.99:
            audio_normalized = audio_normalized / (
                np.abs(audio_normalized).max() + 0.01
            )
...
class DatasetBuilder:
    ...
    def __init__(
        self, preprocessor: AudioPreprocessor, output_format: str = "ljspeech"
    ):
    ...
    def build_from_directory(
        ...
        if num_workers > 1:
            results = self._process_parallel(
                audio_files, wavs_dir, transcripts, speaker_id, num_workers
            )
        else:
            results = self._process_sequential(
                audio_files, wavs_dir, transcripts, speaker_id
            )
    ...
    def _process_sequential(
        ...
        for idx, audio_file in enumerate(tqdm(audio_files, desc="Processing audio")):
            ...
            result = self.preprocessor.process_audio_file(
                audio_file, output_path, transcript
            )
    ...
    def _process_parallel(
        ...
        for idx, audio_file in enumerate(audio_files):
            ...
            tasks.append((audio_file, output_path, transcript, output_name))
                    ...
                    metadata.append(
                        {
                            ...
                            "transcript": result.get("transcript", ""),
                            "normalized_transcript": result.get(
    ...
    def _split_dataset(
        ...
        logger.info(
            f"Dataset split: train={len(train_files)}, val={len(val_files)}, test={len(test_files)}"
        )
    ...
    def _generate_statistics(self, results: List[Dict]) -> Dict:
        ...
        stats = {
            ...
            "total_duration": 0,
            "rejection_reasons": {},
            "quality_metrics": {"snr": [], "energy": [], "duration": []},
        }
