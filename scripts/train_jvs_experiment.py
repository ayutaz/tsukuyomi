...
def optimize_for_4070ti():
    ...
    # 環境変数
    os.environ['PYTORCH_CUDA_ALLOC_CONF'] = 'max_split_size_mb:128'  # メモリフラグメンテーション対策
...
def monitor_memory(step: int, phase: str):
    ...
    if torch.cuda.is_available():
        ...
        logger.info(f"[{phase}] Step {step} - GPU メモリ: "
                   f"使用 {allocated:.2f}GB / 予約 {reserved:.2f}GB / 最大 {max_allocated:.2f}GB")
        
        # メモリ不足の警告
        if reserved > 14.0:  # 16GBの87.5%
            logger.warning("GPUメモリ使用量が高くなっています。バッチサイズの削減を検討してください。")
...
def create_jvs_trainer(config_path: Path, data_dir: Path):
    ...
    if hasattr(config.training, 'memory_optimization'):
        ...
        logger.info(f"  - Gradient Checkpointing: {config.training.memory_optimization.gradient_checkpointing}")
        logger.info(f"  - Clear Cache Interval: {config.training.memory_optimization.clear_cache_interval}")
...
def run_experiment(config_path: Path, data_dir: Path, num_epochs: int = 10):
    ...
    try:
        ...
        if "out of memory" in str(e):
            ...
            logger.error("1. バッチサイズを小さくする (現在: %d)", trainer.config.training.batch_size)
...
def main():
    ...
    if args.profile:
        ...
        with profile(
            ...
            run_experiment(Path(args.config), Path(args.data_dir) if args.data_dir else None, args.num_epochs)
