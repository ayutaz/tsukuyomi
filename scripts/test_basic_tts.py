...
def test_phoneme_encoder():
    ...
    try:
        ...
        phoneme_seqs = ["k o N n i ch i w a", "t s u k u y o m i", "s a N p u r u"]
...
def test_acoustic_model():
    ...
    try:
        ...
        config = AcousticModelConfig(
            hidden_dim=256, n_layers=2, n_heads=4, n_mel_channels=80, use_bf16=False
        )
        ...
        with torch.no_grad():
            outputs = model(
                phoneme_embeddings=phoneme_features, phoneme_mask=phoneme_mask
            )
...
def test_vocoder():
    ...
    try:
        ...
        vocoder = create_vocoder(
            vocoder_type=None, sample_rate=24000, n_mels=80, device="cpu"  # Auto-select
        )
...
def test_end_to_end():
    ...
    try:
        ...
        config = AcousticModelConfig(
            hidden_dim=256, n_layers=2, n_mel_channels=80, use_bf16=False
        )
        ...
        with torch.no_grad():
            outputs = acoustic_model(
                ...
                phoneme_mask=torch.ones(
                    1, phoneme_embeddings.shape[1], dtype=torch.bool
                ),
            )
