...
def create_metadata_csv(metadata_entries: List[Tuple[str, str, str]], output_path: Path):
    """metadata.csvの作成"""
...
def validate_ljspeech_format(output_dir: Path):
    ...
    if missing_wavs:
        print(f"⚠️  メタデータに存在するが音声ファイルがない: {len(missing_wavs)} ファイル")
        
    extra_wavs = wav_filenames - metadata_filenames
    if extra_wavs:
        print(f"⚠️  音声ファイルは存在するがメタデータにない: {len(extra_wavs)} ファイル")
...
def main():
    parser = argparse.ArgumentParser(
        description="JVSデータセットをLJSpeech形式に変換"
    )
    parser.add_argument(
        "--jvs-dir",
        type=str,
    ...
    convert_jvs_to_ljspeech(
        ...
        output_dir,
        args.speakers,
        include_nonpara=args.include_nonpara
    )
