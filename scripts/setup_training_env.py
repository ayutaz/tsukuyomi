...
def check_cuda():
    ...
    if torch.cuda.is_available():
        ...
        print_colored(
            f"✓ CUDA {cuda_version} available with {gpu_count} GPU(s)", Colors.GREEN
        )
        ...
        if cuda_version and float(cuda_version.split(".")[0]) >= 12.1:
            print_colored(
                "  CUDA 12.1+ detected - Flash Attention 2 available", Colors.GREEN
            )
            return True
        else:
            print_colored(
                "  CUDA 12.1+ recommended for optimal performance", Colors.YELLOW
            )
            return True
    else:
        print_colored(
            "✗ CUDA not available. Training will be slow on CPU", Colors.YELLOW
        )
        return False
...
def check_memory():
    ...
    if total_gb >= 32:
        ...
        print_colored(
            f"⚠ System memory: {total_gb:.1f} GB total. 32+ GB recommended",
            Colors.YELLOW,
        )
        return True
...
def check_disk_space():
    ...
    if free_gb >= 100:
        ...
        print_colored(
            f"⚠ Disk space: {free_gb:.1f} GB available. 100+ GB recommended",
            Colors.YELLOW,
        )
        return True
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if "nvidia" in result.stdout.lower():
                ...
                print_colored(
                    "  NVIDIA Docker runtime not found (GPU support disabled in Docker)",
                    Colors.YELLOW,
                )
            return True
...
def check_windows_specific():
    ...
    if platform.system() == "Windows":
        ...
        if vs_path.exists():
            ...
            print_colored(
                "⚠ Visual Studio not found. Required for some packages", Colors.YELLOW
            )
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_colored(
                    "⚠ WSL2 not available. Consider installing for better compatibility",
                    Colors.YELLOW,
                )
        except:
...
def main():
    ...
    print_colored(
        "3. Start training: python scripts/train.py --config configs/train_100h.yaml",
        Colors.WHITE,
    )
