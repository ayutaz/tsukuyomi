    ...
    try:
        ...
        for text in test_texts:
            ...
            outputs = g2p_model.forward(
                text, return_accent=True, return_confidence=True
            )
    ...
    try:
        ...
        for phonemes in test_phonemes:
            ...
            embeddings = xphonebert.encode_japanese_phonemes(
                phonemes, accent_pattern="LHHHL", dialect="tokyo"
            )
        ...
        if xphonebert.config.use_lora:
            ...
            print(
                f"Number of LoRA layers: {len(xphonebert.lora_layers) if hasattr(xphonebert, 'lora_layers') else 0}"
            )
...
def test_bigvgan_v2():
    ...
    try:
        ...
        print(
            f"RTF (Real-Time Factor): {generation_time / (audio.shape[2] / 48000):.3f}"
        )
