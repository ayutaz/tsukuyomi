...
def main():
    ...
    parser.add_argument(
        "--pitch_shift", type=float, default=0.0, help="Pitch shift in semitones"
    )
    parser.add_argument(
    ...
    # Multi-language test
        ...
        for lang, texts in test_texts.items():
            ...
            for i, text in enumerate(texts):
                ...
                start_time = time.time()
                audio, sample_rate = tts.synthesize(
                    text, speaker_id=args.speaker_id, language=lang
                )
                synthesis_time = time.time() - start_time
    ...
    if not args.text and not args.benchmark:
        ...
        audio_list = tts.synthesize_batch(
            batch_texts, speaker_ids=[0, 1, 2], language="ja"
        )
