...
def prepare_jvs_metadata(jvs_dir: Path, speakers: List[str]) -> Dict:
    ...
    for speaker in speakers:
        ...
        speaker_info = {
            "id": speaker,
            "gender": "F" if int(speaker[3:]) <= 50 else "M",  # JVS001-050: 女性, JVS051-100: 男性
            "parallel100": [],
            "nonpara30": [],
            "total_duration": 0.0,
        }
...
def create_file_lists(jvs_dir: Path, metadata: Dict, output_dir: Path, split_ratio: Tuple[float, float, float] = (0.8, 0.1, 0.1)):
    ...
    for split_name, file_list in splits.items():
        ...
        print(f"{split_name}: {len(file_list)} ファイル, {sum(f['duration'] for f in file_list):.1f}秒")


def create_experiment_subset(jvs_dir: Path, output_dir: Path, speakers: List[str], files_per_speaker: int = 50):
    """実験用の小規模サブセットを作成"""
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default=["jvs001", "jvs002", "jvs003", "jvs004", "jvs005",
