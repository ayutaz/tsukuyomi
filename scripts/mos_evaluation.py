...
class MOSDatabase:
    ...
    def add_audio_sample(self, file_path: str, text: str, system_name: str,
                        speaker_id: str = None, emotion: str = None,
                        reference_path: str = None, metadata: Dict = None) -> str:
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (audio_id, file_path, text, system_name, speaker_id, emotion,
              reference_path, json.dumps(metadata) if metadata else None))
    ...
    def add_evaluation(self, evaluator_id: str, audio_id: str, scores: Dict,
                      duration_ms: int = None, comments: str = None):
        ...
        for system in df['system_name'].unique():
            ...
            system_stats[system] = {
                ...
                'aspects': {
                    ...
                    'emotion_appropriateness': system_df['emotion_appropriateness'].mean(),
...
def create_streamlit_app():
    ...
    st.set_page_config(
        page_title="MOS Evaluation Tool",
        page_icon="🎵",
        layout="wide"
    )
    ...
    if st.session_state.evaluator_id:
        ...
        if samples.empty:
            ...
            if st.session_state.current_sample < len(samples):
                ...
                st.header(f"Sample {st.session_state.current_sample + 1} of {len(samples)}")
                ...
                with col1:
                    if st.button("Previous", disabled=st.session_state.current_sample == 0):
...
def main():
    ...
    if args.mode == "prepare":
        ...
        print(f"Overall MOS: {stats['overall']['mean']:.2f} ± {stats['overall']['std']:.2f}")
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) > 1 and sys.argv[1] == "--" and sys.argv[2] == "--mode" and sys.argv[3] == "evaluate":
        create_streamlit_app()
    else:
        main()