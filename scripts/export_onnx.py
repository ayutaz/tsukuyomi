...
def main():
    ...
    parser.add_argument(
        "--opset-version", type=int, default=16, help="ONNX opset version"
    )
    ...
    # Load model
    device = (
        "cuda" if torch.cuda.is_available() and args.target_device == "gpu" else "cpu"
    )
    ...
    if args.model_type == "vits":
        ...
        sample_inputs = {"mel": torch.randn(args.batch_size, 80, 100)}  # [B, n_mels, T]
