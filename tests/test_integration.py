...
class TestTsukuyomiTTSIntegration:
    ...
    def test_batch_synthesis(self, mock_components):
        ...
        mock_components['xphonebert'].batch_encode.return_value = torch.randn(batch_size, 10, 768)
        mock_components['normalizer'].batch_normalize.return_value = [
            {'text': f'normalized {i}', 'phonemes': 't e s u t o', 'tokens': ['te', 'su', 'to']}
    ...
    def test_long_text_synthesis(self, mock_components):
        ...
        mock_components['vocoder'].inference.return_value = np.random.randn(441000)  # 20 seconds
