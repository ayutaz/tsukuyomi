...
from src.server.inference_server import InferenceServer, ServerConfig, TTSRequest, MorphRequest
...
class TestCaching:
    ...
    def server_with_cache(self, mock_tts, redis_mock):
        ...
        with patch('src.server.inference_server.TsukuyomiTTS', return_value=mock_tts):
            with patch('src.server.inference_server.redis.Redis', return_value=redis_mock):
                server = InferenceServer(config)
