...
class TestPerformance:
    ...
    def mock_fast_components(self):
        ...
        with patch('src.inference.JapaneseTextNormalizer') as mock_normalizer, \
            ...
            mock_normalizer_instance.normalize.return_value = {
                'text': 'normalized', 'phonemes': 'p h o n e m e s', 'tokens': ['pho', 'ne', 'mes']
            }
            mock_normalizer_instance.normalize.side_effect = lambda x: (
                time.sleep(0.01), mock_normalizer_instance.normalize.return_value
            )[1]
            ...
            mock_xphonebert_instance.encode.side_effect = lambda *args, **kwargs: (
                time.sleep(0.02), mock_xphonebert_instance.encode.return_value
            )[1]
            ...
            mock_acoustic_instance.return_value = (torch.randn(1, 80, 100), torch.randn(1, 1, 10))
            mock_acoustic_instance.side_effect = lambda *args, **kwargs: (
                time.sleep(0.03), mock_acoustic_instance.return_value
            )[1]
            ...
            mock_vocoder_instance.inference.side_effect = lambda x: (
                time.sleep(0.02), mock_vocoder_instance.inference.return_value
            )[1]
    ...
    def test_batch_performance(self, mock_fast_components):
        ...
        mock_fast_components['xphonebert'].batch_encode.return_value = torch.randn(batch_size, 10, 768)
    ...
    def test_long_text_performance(self, mock_fast_components):
        ...
        for length in text_lengths:
            ...
            mock_fast_components['xphonebert'].encode.return_value = torch.randn(1, length, 768)
            mock_fast_components['acoustic'].return_value = (
                torch.randn(1, 80, length * 10),
                torch.randn(1, 1, length)
            )
            mock_fast_components['vocoder'].inference.return_value = np.random.randn(length * 220)
