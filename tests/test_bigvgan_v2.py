...
class TestBigVGANv2:
    ...
    def test_forward_inference(self, model):
        ...
        assert outputs['audio'].shape[2] == mel_frames * np.prod(model.config.upsample_rates)
...
class TestLossFunctions:
    ...
    @pytest.mark.skipif(not torch.cuda.is_available(), reason="Requires CUDA for librosa mel filters")
