...
class TestOptimizedInferenceEngine:
    ...
    def test_cache_key_computation(self):
        """キャッシュキー計算のテスト"""
        with patch('src.inference.optimized_inference.VITS'), \
             patch('src.inference.optimized_inference.BigVGANv2'):
    ...
    def test_cache_effectiveness(self):
        """キャッシュ効果のテスト"""
        with patch('src.inference.optimized_inference.VITS') as mock_vits, \
            ...
            engine._acoustic_inference_batch = MagicMock(return_value=[torch.randn(1, 128, 100)])
...
class TestTritonModelExporter:
    ...
    def test_pytorch_export(self, mock_trace, temp_model_repo):
        ...
        # エクスポート実行
        exporter._export_pytorch_model(model, temp_model_repo / "test_model/1", "test_model")
